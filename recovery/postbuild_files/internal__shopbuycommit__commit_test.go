...
func TestPersistenceFirstUnderLockOrder(t *testing.T) {
	...
	got, err := PersistenceFirstUnderLock(&mu, allowedGate(),
		...
		func(value int) error {
			...
			if value != 7 { t.Fatalf("persist value=%d", value) }
			events = append(events, "persist")
			return nil
		},
		func(value int) {
			assertMutexHeld(t, &mu, "apply")
			if value != 7 { t.Fatalf("apply value=%d", value) }
			events = append(events, "apply")
		},
	)
	if err != nil { t.Fatal(err) }
	if got != 7 { t.Fatalf("got=%d", got) }
...
func TestPersistenceFirstUnderLockGateBlocksBeforeCallbacks(t *testing.T) {
	...
	if called { t.Fatal("blocked gate reached callbacks") }
	assertMutexReleased(t, &mu)
}
...
func TestPersistenceFirstUnderLockStageFailureSkipsPersistenceAndApply(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "stage boom") { t.Fatalf("err=%v", err) }
	if persisted || applied { t.Fatalf("persisted=%t applied=%t", persisted, applied) }
	assertMutexReleased(t, &mu)
}
...
func TestPersistenceFirstUnderLockPersistenceFailureSkipsApply(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "db boom") { t.Fatalf("err=%v", err) }
	if applied { t.Fatal("persistence failure reached live apply") }
	assertMutexReleased(t, &mu)
}
