...
func TestOrdinaryShopBagConflictResyncRealMySQL(t *testing.T) {
	...
	if dsn == "" { t.Fatal("disposable shop MySQL DSN missing") }
	db, err := sql.Open("mysql", dsn)
	if err != nil { t.Fatal(err) }
	defer db.Close()
	var databaseName string
	if err := db.QueryRow("SELECT DATABASE()").Scan(&databaseName); err != nil { t.Fatal(err) }
	if databaseName != "shop_atomic_ci" { t.Fatalf("refusing non-disposable database %q", databaseName) }
...
hardiness INT NULL, max_hardiness INT NULL, PRIMARY KEY(role_id,seq)) ENGINE=InnoDB`,
	...
	if err != nil { t.Fatal(err) }
	if _, err := db.Exec(`INSERT INTO roles(role_id) VALUES (?)`, roleID); err != nil { t.Fatal(err) }
	if _, err := db.Exec(`INSERT INTO role_currency(role_id,snapshot) VALUES (?,?)`, roleID, beforeJSON); err != nil { t.Fatal(err) }
	...
	for seq, row := range originalRows {
		if _, err := db.Exec(`INSERT INTO role_bag_items(role_id,seq,slot,config_id,item_type,amount,view_id) VALUES (?,?,?,?,?,?,?)`, roleID, seq, row.Slot, row.ConfigID, row.ItemType, row.Amount, row.ViewID); err != nil { t.Fatal(err) }
	}
	...
	if !reflect.DeepEqual(actor.bagSnapshot(), newer) { t.Fatalf("actor bag not DB state: %+v", actor.bagSnapshot()) }
	if len(capture.frames) != 3 { t.Fatalf("view replay contains %d frames, want 3", len(capture.frames)) }
	var walletJSON []byte
	if err := independent.QueryRow(`SELECT snapshot FROM role_currency WHERE role_id=?`, roleID).Scan(&walletJSON); err != nil { t.Fatal(err) }
	var persistedWallet currencySnapshot
	if err := json.Unmarshal(walletJSON, &persistedWallet); err != nil { t.Fatal(err) }
	if persistedWallet != before { t.Fatalf("rejected buy charged currency: %+v", persistedWallet) }
	persistedBag, ok := (&mysqlBagStore{db: independent}).Load(role.RoleID(roleID))
	if !ok || !reflect.DeepEqual(persistedBag, newer) { t.Fatalf("independent reconnect bag changed: %+v ok=%t", persistedBag, ok) }
	...
	persistedBag, ok = (&mysqlBagStore{db: independent}).Load(role.RoleID(roleID))
	if !ok || !reflect.DeepEqual(persistedBag, fresh) { t.Fatalf("explicit purchase did not persist once: %+v", persistedBag) }
	if err := independent.QueryRow(`SELECT snapshot FROM role_currency WHERE role_id=?`, roleID).Scan(&walletJSON); err != nil { t.Fatal(err) }
	if err := json.Unmarshal(walletJSON, &persistedWallet); err != nil { t.Fatal(err) }
	if persistedWallet != after { t.Fatalf("explicit purchase wallet=%+v want=%+v", persistedWallet, after) }
}
