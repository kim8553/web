...
- `server/cmd/protocol-probe/main.go`: an unset `NINEYIN_MYSQL_DSN` selects legacy JSON storage. A configured MySQL connection automatically calls `migrations.Runner.Up`. Neither user-schema compatibility nor permission to migrate has been established. Never run this against a user's DB without independent schema review, backup and explicit approval.
...
3. `migrations.Runner.Up` auto-migration safety and the user's existing database schema must be reconciled without running migrations on user DB without approval; do not ask user to set the bypass flag.
4. Prior NPC shop purchase gameplay implementation must remain intact; no NPC selling, GM item grants, or exchange work is authorized by this startup task.
