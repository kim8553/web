...
FINAL_MANIFEST_SHA='8a9a2f763948f7fd2735969720a5dd7e81e8d1f55c67131d897fad106874d5b3'

# First reproduce and fully verify the already-passed 199-file checkpoint.
bash tools/stage37_current_recovery_stage2.sh

# Add stale-identity metadata to staged deductions and a pure cloned-bag apply
# layer. These files still do not mutate player state or persistence.
echo '2009d83c1940ed375a8a7c6aa6b0833e62114d85e0df3c55001222a75de9680c  recovery/postbuild_files/internal__exchangeplan__replacement_v2.go' | sha256sum -c -
echo '829ec1a53c54beb398f634d9fd7758bcb6022f035ed8f959853dad13257645af  recovery/postbuild_files/cmd__protocol-probe__latest_client_shop_exchange_apply.go' | sha256sum -c -
echo '757768fdab6273857a3a40fbea4585b1860eedcb72ac0f4091888eed3a5c0f9a  recovery/postbuild_files/cmd__protocol-probe__latest_client_shop_exchange_apply_test.go' | sha256sum -c -
cp recovery/postbuild_files/internal__exchangeplan__replacement_v2.go "$BUILD/internal/exchangeplan/replacement.go"
cp recovery/postbuild_files/cmd__protocol-probe__latest_client_shop_exchange_apply.go "$BUILD/cmd/protocol-probe/latest_client_shop_exchange_apply.go"
cp recovery/postbuild_files/cmd__protocol-probe__latest_client_shop_exchange_apply_test.go "$BUILD/cmd/protocol-probe/latest_client_shop_exchange_apply_test.go"
find "$BUILD/cmd" "$BUILD/internal" -name '*.go' -print0 | xargs -0 gofmt -w

# Exclude CI-only files/build output while hashing the exact 201-file source.
test -f "$BUILD/ci.real.mod"
test -f "$BUILD/ci.real.sum"
test -f "$BUILD/protocol-probe"
mv "$BUILD/ci.real.mod" "$ROOT/ci.real.mod.stage3"
mv "$BUILD/ci.real.sum" "$ROOT/ci.real.sum.stage3"
rm "$BUILD/protocol-probe"
test "$(find "$BUILD" -type f | wc -l | tr -d ' ')" = '201'
...
mv "$ROOT/ci.real.mod.stage3" "$BUILD/ci.real.mod"
mv "$ROOT/ci.real.sum.stage3" "$BUILD/ci.real.sum"

# Re-run all production gates against the final 201-file tree.
cd "$BUILD"
set +e
run_gate() {
  local name="$1"
