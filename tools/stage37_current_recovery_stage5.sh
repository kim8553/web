...
# Reproduce the already-verified 203-file production checkpoint first.
bash tools/stage37_current_recovery_stage4.sh

# Add the resource-independent persistence-first primitive, then replace the
# dormant protocol helper with the version that delegates sequencing to it.
cp recovery/postbuild_files/internal__exchangecommit__persist.go "$BUILD/internal/exchangecommit_persist.tmp"
mkdir -p "$BUILD/internal/exchangecommit"
mv "$BUILD/internal/exchangecommit_persist.tmp" "$BUILD/internal/exchangecommit/persist.go"
cp recovery/postbuild_files/internal__exchangecommit__persist_test.go "$BUILD/internal/exchangecommit/persist_test.go"
cp recovery/postbuild_files/cmd__protocol-probe__latest_client_shop_exchange_commit_v2.go "$BUILD/cmd/protocol-probe/latest_client_shop_exchange_commit.go"
gofmt -w "$BUILD/internal/exchangecommit" "$BUILD/cmd/protocol-probe/latest_client_shop_exchange_commit.go"

# Stage4 leaves CI-only module files and a Linux build output in buildtree.
test -f "$BUILD/ci.real.mod"
test -f "$BUILD/ci.real.sum"
test -f "$BUILD/protocol-probe"
mv "$BUILD/ci.real.mod" "$ROOT/ci.real.mod.stage5"
mv "$BUILD/ci.real.sum" "$ROOT/ci.real.sum.stage5"
rm "$BUILD/protocol-probe"
test "$(find "$BUILD" -type f | wc -l | tr -d ' ')" = '205'
...
mv "$ROOT/ci.real.mod.stage5" "$BUILD/ci.real.mod"
mv "$ROOT/ci.real.sum.stage5" "$BUILD/ci.real.sum"
...
# These two gates execute the persistence-first semantics without touching the
# resource-bound protocol package.
...
failed=0
for x in exchangecommit exchangecommit_race planner migrations build protocol_compile protocol_race_compile nonprotocol race vet windows; do
