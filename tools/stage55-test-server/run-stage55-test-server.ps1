# Stage55 launcher for the current source lineage descended from 9yin-go-server1.rar.
[CmdletBinding()]
param(
...
$exe = Join-Path $PSScriptRoot 'stage55-test-server.exe'
if (-not (Test-Path -LiteralPath $exe -PathType Leaf)) {
    Write-Host 'BLOCKED: stage55-test-server.exe is missing.'
    exit 2
}
...
try {
    ...
    Write-Host 'Starting Stage55 test server from the current 9yin-go-server1 source lineage.'
    ...
    Write-Host 'Missing optional playerweapon helper directory no longer blocks startup.'
